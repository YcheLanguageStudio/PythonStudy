...
def depth_first_search(my_huffman_nodes):
    if len(my_huffman_nodes) == 1:
        if re.match('.*[Aa]nswer.*', recover_message(my_huffman_nodes[0])):
            print recover_message(my_huffman_nodes[0])
            return True
        else:
            return False
    else:
            ...
            for i in range(0, max_same_index + 1):
                for j in range(0, max_same_index + 1):
                    if i != j:
                        ...
                        if depth_first_search(copy_my_huffman_nodes):
                            return True
        else:
...
def process_huffman_nodes(my_huffman_nodes, tmp_result_list):
    reduce_two_huffman_nodes(my_huffman_nodes, 0, 1)
    tmp_result_list.append(my_huffman_nodes)
    if my_huffman_nodes[0].frequency == my_huffman_nodes[1].frequency:
        max_same_index = 1
        for i in range(2, len(my_huffman_nodes)):
            if my_huffman_nodes[i].frequency == my_huffman_nodes[1].frequency:
...
# deprecated
def build_huffman_tree_bfs(huffman_node_list):
    result_list = list()
...
def print_dfs(tree):
    if tree.left_node is None and tree.right_node is None:
        ...
        print tree
        print 'left:'
        print_dfs(tree.left_node)
        print 'right:'
        print_dfs(tree.right_node)
        print '--'
...
def recover_message(candidate_tree):
    ...
    for idx in range(0, len(encoded_text)):
        if tmp_node.left_node is None and tmp_node.right_node is None:
            ret_list.append(tmp_node.letter)
            tmp_node = candidate_tree
...
if __name__ == '__main__':
    ...
    depth_first_search(huffman_nodes)
